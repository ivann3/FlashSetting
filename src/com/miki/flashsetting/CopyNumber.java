...
/**
 * 
 * @author Shenl 2016.10.28 V02版
 */
public class CopyNumber extends Activity {

	private final static String TAG = "SHEN_LEI";
	
	@SuppressWarnings("unused")
	private Button clean, end, delete, number, autoCopy;
	private TextView statusView, pro_tv, tv_free, progress_tv, progress_tv1,
			current;
	...
	public void initView() {
		mContext = this;
		statusView = (TextView) findViewById(R.id.text_view);
		...
		case R.id.number:
			...
			if (mk == 0) {
				Toast.makeText(this, "默认复制100次", Toast.LENGTH_SHORT)
						.show();
				statusView.setText("当前次数： 100");
			}else{
				Toast.makeText(this, "设置" + mk + "次成功", Toast.LENGTH_SHORT)
				.show();
				statusView.setText("当前次数： " + mk);
			}
			break;
		case R.id.autoCopy:
			isStop = true;
...
//			Toast.makeText(this, "复制" + mk + "次", Toast.LENGTH_SHORT).show();
			statusView.setText("复制中.....");
			break;
		}
	}
	...
	public void cleanFile(){
		...
		pro_tv.setText("");
		File cleanFile = new File(path,"data.txt");
		try {
			if(!cleanFile.exists()){
	...
	public void deleteSD() {
		...
		statusView.setText("");
		currentNum++;
		current.setText("完成" + currentNum + "次");
...
//				}
	...
	public long getSDFreeSize() {
		...
		progress_tv.setText("可用内存比例：" + freeSize + "%");
